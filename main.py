...
# # Plotting the distribution of a few key numerical features
...
# # Assuming df is your DataFrame and the variables are named 'avganncount', 'avgdeathsperyear', and 'incidencerate'
#
# # Creating a new composite index
# df['cancer_impact_index'] = (df['avganncount'] + df['avgdeathsperyear'] + df['incidencerate']) / 3
#
# # Now drop the original columns from your DataFrame
...
# Imputing missing values in the remaining columns with their respective mean values
...
class NeuralNetworkHyperModel(HyperModel):
    ...
    def build(self, hp):
        ...
        for i in range(hp.Int('num_layers', 2, 6)):  # Example: between 1 and 5 layers
            # Adding layers
            if i == 0:
...
tuner = RandomSearch(
    hypermodel,
    objective='val_loss',
    max_trials=25,  # Number of trials to run
    executions_per_trial=4,  # Number of models to build and fit for each trial
...
top_correlated_columns = correlations.index[1:5]  # Change 5 to however many you want

# # Plotting scatter plots for each of the top correlated variables against target_deathrate
...
# plt.show()
#
# # Calculating R-squared and Root Mean Squared Error (RMSE) for the neural network model
# r_squared_nn = r2_score(y_test_nn, y_pred_nn)
# rmse_nn = np.sqrt(mean_squared_error(y_test_nn, y_pred_nn))
#
# # Implementing 10-fold cross-validation for the neural network model
# cv_scores_nn = cross_val_score(nn_model, X, data_cleaned['target_deathrate'], cv=10, scoring='neg_root_mean_squared_error')
#
# # Negating the cross_val_score function's negative RMSE values for the neural network model
# cv_rmse_scores_nn = -cv_scores_nn
#
# # Calculating the average RMSE across all folds for the neural network model
...
#
#
# # Calculating R-squared and Root Mean Squared Error (RMSE) for the best decision tree model
...
#
# # Calculating R-squared and Root Mean Squared Error (RMSE) for the Gradient Boosting model
...
# param_dist = {
#     'n_estimators': (349, 351),  # Randomly sample values between 100 and 500
#     'max_depth': [None] + list(range(8, 9)),  # None or any integer between 3 and 10
#     'min_samples_split': sp_randint(9, 10),  # Randomly sample values between 2 and 10
#     'min_samples_leaf': sp_randint(1, 2),  # Randomly sample values between 1 and 4
...
# n_iter_search = 3  # Number of hyperparameter settings that are sampled
...
# plt.legend()
# plt.show()
# # Calculating R-squared and Root Mean Squared Error (RMSE) for the best Random Forest model
...
# Calculating R-squared and Root Mean Squared Error (RMSE)
...
#
# # Calculating R-squared and Root Mean Squared Error (RMSE) for the AutoKeras model
